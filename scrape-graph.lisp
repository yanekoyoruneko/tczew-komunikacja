...
The row with the next station is the one after the first one highlighted with color #000"
  (multiple-value-bind (data url) (scrapycl:fetch spider arrive-time-req)
    (declare (ignore url))
    (list (make-instance 'arrive-time :minutes
			 (parse-integer
			  (lquery:$1 (initialize data) "tr" (has "[color=\"#000\"]") (eq 1)
				     "td" (eq 2) (text))
			  :junk-allowed t)))))
